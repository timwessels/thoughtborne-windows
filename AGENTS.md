...
- **Verification is a ladder.** Bottom rung: `python -m py_compile <changed files>`. Middle rung: `python3 run_tests.py` runs every `test_*.py` in the repo root off-Windows (stdlib plus `soundfile` and `groq`), prints one verdict line per driver, exits non-zero if any failed, and surfaces each driver's `(skipped ...)` notes so a green run never reads as if a skipped check had passed. CI runs the same command on every push and pull request ([`.github/workflows/tests.yml`](.github/workflows/tests.yml), Python 3.10 + 3.12 under `xvfb-run`, so the display-gated checks run for real). Top rungs, hands-on only: start the tool and check the startup masthead plus `All hotkeys registered successfully` in `thoughtborne.log`, then let the in-app self-test `Ctrl+Alt+T` transcribe `test_audio.mp3` end to end (needs a valid API key).
...
| `test_settings_visibility.py` | The tkinter-free visibility helpers and the `[SETTINGS]` log lane; with a display, regressions against the real app (auto-hide, wrapping, maximize→restore, language toggle, the reset control, crash lanes, and the tab strip fitting the minimum window width). | `--show`; display parts skip without a display |
