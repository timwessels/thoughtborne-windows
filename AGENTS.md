...
- **Hotkey keys are static-only.** Every bindable key — shipped default or user override — is a letter, digit, or F-key from `hotkey_parse.VK_MAP`; anything else is rejected at config time with a log warning and the action keeps its default. The former `ü` / `VkKeyScanW` override lane was removed on purpose (D-023, completing D-012) — do not reintroduce a layout-resolved key lane.
