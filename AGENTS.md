...
| `test_config_loading.py` | The hardened `personal_settings.json` and `.env` readers against tempdir fixtures, each re-checked by a real `import config` in a subprocess, plus the top-level block gate with the drift guards on its known-blocks tuple, the `push_to_talk` / `soniox_endpointing` value rules and the vocabulary's arrival in both real Soniox requests, three frozen historical settings files through the whole load path (D-026), the one `.env` parser both halves share, the guards that keep the process environment out (D-017), the `pyproject.toml` version reader with its regex twin in `setup.ps1`, and the checkout state read from the `.git` beside the script, fail-open in every shape git writes it. | `--show`; skips its `groq` lanes when that is absent, and its real-repository case where there is no `.git` |
| `test_console_ui.py` | Every console panel/strip: widths, CP437 charset, the plain-ASCII twin, red-exclusivity, the key-aware lineup and the keyless panels, the mixed-scheme fixtures for every key-bearing surface, the combo stress check, the static guard that the app derives no key of its own (D-019), the app ↔ fixture seam measured in both directions, the values a surface must show, and that a flag's two branches still differ. | `--show` |
...
| `test_hotkey_suspend_wiring.py` | The glue of the #335 hotkey suspend the pure decision cannot see: the real `_hotkey_suspend_tick` against a scriptable manager and a real request file — one release per capture, a refused post retried in both directions, the failsafe withdrawing the request before it resumes, no flapping on an undeletable one — plus the manager's two private messages, its post gate, its contained completion hooks, and the real message pump proving the ACK is off disk before the first registration comes back. | — |
...
| `test_ptt_ownership.py` | The app-side ownership lifecycle the detector cannot see (#250): the real `_ptt_tick` / start / stop-and-insert methods against a fake app with `is_vk_pressed` scripted — a recording ended through a foreign door releases ownership and resets the detector, and the owned round-trip is unchanged. | — |
| `test_restart_signal.py` | Both settings-app ↔ tool handshakes: the restart signal's write/consume round-trip with the no-shutdown-without-a-successful-consume invariant, and the #335 suspend pair — its two filenames, its writers, and the truth table of the pure `decide_hotkey_suspend`. | — |
...
| `test_settings_io.py` | The settings-app IO core: the surgical `.env` / `personal_settings.json` merges with the `.env` quoting round-trip and the WYSIWYG key deletion, the hotkey-combo helpers, `key_check`, the DE/EN string table with every key the app hands a text sink, the pure save/engine decision helpers, and the D-026 backup lane (backup before loss, no backup → no overwrite). | `--show` |
...
| `test_settings_visibility.py` | The tkinter-free visibility helpers, the `[SETTINGS]` log lane, and the regressions driven against the real app (auto-hide, wrapping, maximize→restore, language toggle, the reset control, the D-026 save/reset backup lanes, the WYSIWYG `.env` key deletion, the #335 capture arm wait (mocked tool, hand-turned clock), the key-agnostic engine picker (D-028), crash lanes, the tab strip fitting the minimum window width, and no placed text element left blank). | `--show` |
...
| `test_startup_carousel.py` | The startup carousel's fall-through (#40) through the real `_create_startup_transcriber` — what D-028 lets a keyless pin rely on: a pin or memory on a keyless engine starts the first keyed engine, a keyed start constructs exactly once, an all-keyless start ends in the #200 shop window, and no case writes the engine memory (D-008). | — |
| `test_stop_rebind_wiring.py` | That the release-wait lists, the mis-trigger net and the per-action stop debounce really derive from the effective `HOTKEYS` at call time (#152) — the real hotkey callbacks against a fake app, with `HOTKEYS` rebound and `is_key_pressed` scripted; the deliver kwargs each of the five stop flows hands over, negatives included (#330); plus a static guard against literal wait lists. | — |
...
- **Hotkey keys are static-only.** Every bindable key — shipped default or user override — is a statically mapped key from `hotkey_parse.VK_MAP`, bindable bare or with modifiers (D-027); anything else is rejected at config time with a log warning and the action keeps its default. The former `ü` / `VkKeyScanW` override lane was removed on purpose (D-023, completing D-012) — do not reintroduce a layout-resolved key lane.
...
- **Source:**
  ...
  - `restart_signal.py` — the settings-app ↔ tool handshakes (#202 restart, #335 hotkey suspend, one signal file each way) and the tool's single-instance mutex name, the one source both sides share.
- **Settings app:**
...
- **User data** (created at runtime, gitignored): `history/` (recordings in `audio/`, transcripts in `transcripts/`), `thoughtborne.log`, `runtime_state.json` (machine-written, but it carries the user's last engine choice), and any `personal_settings.backup-*.json` (D-026's backup-before-loss copies, never auto-deleted) — never delete. Older checkouts may still carry the legacy `voice_archive/` + `text_archive/` folders (auto-migrated into `history/` at startup) — never delete those either.
