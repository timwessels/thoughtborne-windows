...
| `test_console_ui.py` | Every console panel/strip: widths, CP437 charset, the plain-ASCII twin, red-exclusivity, the key-aware lineup and the keyless panels, the mixed-scheme fixtures for every key-bearing surface, the combo stress check, the static guard that the app derives no key of its own (D-019), the one holding the registration counts apart from the mouse listeners (#308), the app ↔ fixture seam measured in both directions, and the values a surface must show. | `--show` |
...
| `test_hotkey_overrides.py` | The `personal_settings.json` hotkey override surface — `hotkey_parse` and `config.apply_hotkey_overrides` — plus the guard that every shipped default is a statically mapped key (D-012), the one holding the README twins' `## Hotkeys` tables to `DEFAULT_HOTKEYS`, order and combos (D-019), and the mouse-button grammar with its bare-only rule, asserted against both acceptance authorities at once (#308). | `--show` |
| `test_mouse_detector.py` | The mouse-hotkey press-edge state machine against synthetic tick sequences, compared as exact per-tick id lists. | — |
...
| `test_settings_io.py` | The settings-app IO core: the surgical `.env` / `personal_settings.json` merges, the hotkey-combo helpers, both capture decoders (the key one and the mouse one, whose number map carries the Tk 8.6 and Tk 9 button numbering, #308), `key_check`, the DE/EN string table with every key the app hands a text sink, the pure save/engine decision helpers. | `--show` |
...
- **Source:**
  ...
  - `mouse_detector.py` — the mouse-hotkey press-edge state machine, Win32-decoupled: `RegisterHotKey` accepts a mouse button and then never fires for it, so mouse combos are polled instead — on their own 10 ms thread (`thoughtborne.mouse_hotkey_thread`, never the audio-paced recording loop) and posted to the listener thread (#308).
