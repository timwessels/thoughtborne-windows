...
| `test_settings_io.py` | The settings-app IO core: the surgical `.env` / `personal_settings.json` merges, the hotkey-combo helpers, `key_check`, the DE/EN string table with every key the app hands a text sink, the pure save/engine decision helpers, and the D-026 backup lane (backup before loss, no backup → no overwrite). | `--show` |
