...
**Hotkeys & Engine beim Start (optional):** Die Tastenkombinationen und die beim Start gewählte Engine lassen sich in der `personal_settings.json` überschreiben, ohne Code zu ändern — ein `hotkeys`-Block (Teil-Überschreibung nach Aktionsnamen; F-Tasten `f1`–`f24` sind erlaubt, auch modifier-lose wie `f9`, und ebenso drei Maustasten — `mbutton` für den Mausrad-Klick, `xbutton1` und `xbutton2` für die Daumentasten, immer ohne Modifier) und ein `defaults`-Block (`"api"`, eine der vier Engines). `config.py` behält die eingebauten Defaults; eine gelistete Aktion oder eine gültige `api` ersetzt sie, und eine unbekannte Aktion, eine nicht parsbare Kombination, eine Kollision oder eine unbekannte Engine wird mit einer Warnung in `thoughtborne.log` ignoriert, sodass das Tool immer startet. Die Kommentare in `personal_settings.example.json` listen die gültigen Aktionsnamen und das Format. Eine Maustaste ist ein Hotkey wie jeder andere, mit einer Ausnahme: Thoughtborne kann andere Programme nicht davon abhalten, ebenfalls auf sie zu reagieren — wer die Zurück-Taste vergibt, navigiert im Explorer und im Browser weiterhin eine Seite zurück —, weshalb der saubere Weg darin besteht, die Maus-Software des Herstellers eine Tastenkombination senden zu lassen und die hier zu belegen; die Einstellungs-App nimmt eine Maustaste wie eine Taste auf und sagt dasselbe einmal, im Moment der Vergabe.
