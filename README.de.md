...
**Hotkeys & Engine beim Start (optional):** Die Tastenkombinationen und die beim Start gewählte Engine lassen sich in der `personal_settings.json` überschreiben, ohne Code zu ändern — ein `hotkeys`-Block (Teil-Überschreibung nach Aktionsnamen) und ein `defaults`-Block (`"api"`, eine der vier Engines). Belegbare Tasten: Buchstaben, Ziffern, F-Tasten `f1`–`f24`, der Navigationsblock (`insert`, `delete`, `home`, `end`, `pageup`, `pagedown`), die Pfeiltasten (`left`, `up`, `right`, `down`), der Ziffernblock (`num0`–`num9`, `numdecimal`, `numdivide`, `nummultiply`, `numsubtract`, `numadd`), `pause` und `scrolllock` — jede davon blank (wie `f9` oder `pause`) oder mit `ctrl`/`alt`/`shift`/`win`. `config.py` behält die eingebauten Defaults; eine gelistete Aktion oder eine gültige `api` ersetzt sie, und eine unbekannte Aktion, eine nicht parsbare Kombination, eine Kollision (mit einer bewussten Ausnahme: Die Start-Kombination darf zusätzlich auf genau einer Stopp-Aktion liegen — der Umschalter weiter unten) oder eine unbekannte Engine wird mit einer Warnung in `thoughtborne.log` ignoriert, sodass das Tool immer startet. Die Kommentare in `personal_settings.example.json` listen die gültigen Aktionsnamen und das Format. Fünf Dinge, die man über die Sondertasten wissen sollte:
