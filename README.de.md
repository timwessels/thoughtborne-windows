...
Die Konfiguration hat eine grafische Eingangstür: eine kleine Einstellungs-App — ein Fenster, das zugleich Erststart-Assistent und alltäglicher Einstellungsdialog ist, auf Deutsch oder Englisch (umschaltbar im Kopfbereich). Beim ersten Start ohne API-Key öffnet Thoughtborne sie automatisch und führt durch ihre Seiten — Transkriptionsanbieter & API-Key (mit Live-Prüfung „Key testen"), Hotkeys (Push-to-talk an oder aus, zwei Ein-Klick-Preset-Schemata oder jede Kombination pro Aktion per Tastendruck aufnehmen), Start & Fenster (die Engine beim Start, dazu Wegweiser für Tray und Admin-Fenster), Maschinenraum (diese Installation: Version, Installationsordner, die zwei eigenen Dateien, Aktualisieren, Lizenz und eine Schaltfläche, die die Einstellungen auf den Auslieferungszustand zurücksetzt — deine API-Keys und alles, was du selbst in die `personal_settings.json` geschrieben hast, bleiben). Später, sobald ein Key gespeichert ist, öffnest du dasselbe Fenster jederzeit aus dem laufenden Tool mit **`Ctrl+Alt+G`** — es ist Teil von Thoughtborne, kein eigenes Programm. Speichern heißt **Speichern & neu starten**: Es speichert, startet das Tool für dich neu, und die Änderungen greifen sofort — du musst nicht daran denken, es selbst neu zu starten (eine laufende Aufnahme wird vorher gesichert und lässt sich nach dem Neustart mit dem Wiederhol-Hotkey erneut transkribieren). Ohne Key gilt das genauso — das Tool kommt zurück und öffnet dieses Fenster wieder, bis ein Key eingetragen ist. Beendet sich das Tool nicht innerhalb weniger Sekunden, sagt sie das und lässt es unangetastet weiterlaufen; es wird nie etwas erzwungen geschlossen.

<p align="center"><img src="assets/screenshots/settings-provider.png" width="520" alt="Die Einstellungs-App im Reiter Provider &amp; API key: oben die Erklärung, was ein API-Key ist, darunter eine Karte für Groq — der kostenlose Weg — mit verdecktem Key-Feld und den Schaltflächen Show und Test key, darunter eine Karte für Soniox — der Qualitäts-Weg."></p>

Die App schreibt dieselben zwei Dateien, die sich auch von Hand pflegen lassen — `.env` (die Keys) und `personal_settings.json` (Hotkeys, Engine beim Start, Push-to-talk) — und bearbeitet sie chirurgisch: Handgepflegtes wie das Erkennungs-Vokabular und jeder Kommentar bleiben unangetastet, und Hotkeys landen nur dort in der Datei, wo sie von den Defaults abweichen. Ein laufendes Thoughtborne übernimmt Änderungen beim nächsten Start (kein Live-Reload). Alles, was die App tut, bleibt von Hand editierbar — sie ist ein Frontend für die genannten Dateien, kein Ersatz (siehe [Anpassung](#anpassung)).
...
**Hotkeys & Engine beim Start (optional):** Die Tastenkombinationen und die beim Start gewählte Engine lassen sich in der `personal_settings.json` überschreiben, ohne Code zu ändern — ein `hotkeys`-Block (Teil-Überschreibung nach Aktionsnamen) und ein `defaults`-Block (`"api"`, eine der vier Engines). Belegbare Tasten: Buchstaben, Ziffern, F-Tasten `f1`–`f24`, der Navigationsblock (`insert`, `delete`, `home`, `end`, `pageup`, `pagedown`), die Pfeiltasten (`left`, `up`, `right`, `down`), der Ziffernblock (`num0`–`num9`, `numdecimal`, `numdivide`, `nummultiply`, `numsubtract`, `numadd`), `pause` und `scrolllock` — jede davon blank (wie `f9` oder `pause`) oder mit `ctrl`/`alt`/`shift`/`win`. Zwei Arten von Kombinationen sind davon ausgenommen, weil die Texteingabe selbst auf ihnen liegt. Das blanke `ctrl+v` ist das Einfügen, das Thoughtborne selbst sendet, um ein Transkript einzusetzen — ein Hotkey darauf schluckt die eigene Ausgabe des Tools. Und `ctrl+alt` mit `q`, `e`, `m`, `2`, `3`, `7`, `8`, `9` oder `0` ist genau das, was das Tippen von `@ € µ ² ³ { [ ] }` auf deutschen Tastaturen sendet, denn AltGr **ist** Strg+Alt — ein Hotkey darauf schluckt dieses Zeichen in jeder Anwendung. Beide Arten werden mit einer Warnung abgelehnt, die den Mechanismus nennt, und die Aktion behält ihren Default; alles andere bleibt deine Entscheidung, auch ein blanker Buchstabe oder eine Kombination, die das Kürzel eines anderen Programms überdeckt. `config.py` behält die eingebauten Defaults; eine gelistete Aktion oder eine gültige `api` ersetzt sie, und eine unbekannte Aktion, eine nicht parsbare Kombination, eine Kollision (mit einer bewussten Ausnahme: Die Start-Kombination darf zusätzlich auf genau einer Stopp-Aktion liegen — der Umschalter weiter unten) oder eine unbekannte Engine wird mit einer Warnung in `thoughtborne.log` ignoriert, sodass das Tool immer startet. Die Kommentare in `personal_settings.example.json` listen die gültigen Aktionsnamen und das Format. Fünf Dinge, die man über die Sondertasten wissen sollte:

- Rollen (Scroll Lock) schaltet seinen Sperrzustand (und die Tastatur-LED) weiterhin um, wenn es als Hotkey feuert.
- Pause und Rollen lassen sich nicht mit Ctrl kombinieren — bei gehaltenem Ctrl liefert Windows einen anderen Tastencode, eine solche Kombination könnte also nie feuern; sie wird wie jeder ungültige Eintrag mit einer Warnung abgelehnt.
- Ein Ziffernblock-Hotkey braucht eingeschaltetes Num Lock; bei ausgeschaltetem Num Lock senden dieselben physischen Tasten stattdessen die Navigations-Codes — Ziffernblock-7 löst dann einen auf `home` gelegten Hotkey aus.
- Shift verträgt sich nicht mit den Ziffern des Ziffernblocks und der Komma-Taste: Gehaltenes Shift setzt Num Lock aus, die Taste kommt als ihr Navigations-Zwilling an, und die Kombination feuert nie — stillschweigend, denn sie registriert sich wie jede andere.
- In einer Remote-Desktop-Sitzung sind `Ctrl+Alt+Home`/`End`/`Pause` und `Ctrl+Alt` + Ziffernblock-Plus/Minus eigene Kürzel des RDP-Clients und erreichen das Tool nicht.

**Eine Taste für Start und Stopp (optional):** Die Kombination von `start_recording` lässt sich zusätzlich auf genau eine der vier Stopp-Aktionen legen — dann wird diese Taste zum Umschalter: einmal drücken startet die Aufnahme, noch einmal drücken beendet sie und liefert den Text genau so aus, wie es die eigene Taste dieser Stopp-Aktion täte, und der nächste Druck startet die nächste Aufnahme. Das ist die einzige Dopplung, die die Kollisionsregel zulässt: Der Aufnahmezustand entscheidet, welche der beiden Aktionen feuert, es geht also nichts still verloren.

```json
"hotkeys": { "start_recording": "pause", "stop_recording_clipboard": "pause" }
```

Es genügt auch, nur die Stopp-Hälfte einzutragen — dann wird die ausgelieferte Start-Kombination zum Umschalter (`"stop_recording_clipboard": "ctrl+alt+w"`). Zwei Folgen gehören zum Entwurf dazu: Die eigene Standard-Kombination der Partner-Aktion (hier `Ctrl+Alt+A`) ist danach nicht mehr belegt, und diese Aktion fügt den letzten Text nicht mehr erneut ein, wenn man sie außerhalb einer Aufnahme drückt — ein Druck startet dann immer eine neue Aufnahme. Das nachträgliche Einfügen bleibt über die übrigen Stopp-Tasten verfügbar.
...
- `HOTKEYS` — die Standard-Tastenkombinationen. Zum Ändern besser den `hotkeys`-Block der `personal_settings.json` nutzen (oben); `config.py` hält die Defaults. Tasten sind Buchstaben, Ziffern, F-Tasten, Navigations- und Pfeiltasten, der Ziffernblock, Pause und Rollen — alle mit festen Tastencodes auf jedem Layout. Alles andere (Sonderzeichen wie `#`, Nicht-ASCII-Buchstaben) wird mit einer Warnung in `thoughtborne.log` abgelehnt, und der Default bleibt.
