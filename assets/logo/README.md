...
| `make_app_icon.py` | Rebuilds `thoughtborne.ico` from `console_ui.LOGO_MARK_A5` and `console_ui.ACCENT` (needs Pillow; build-time only) — run it when the mark or the accent changes. **Changing the icon's look? Ship it under a new filename** — Windows' icon caches key on the path and demonstrably survive a content change (field-tested 2026-09-15); the plan lives in #323 |
