...
- **Recognition vocabulary** (recommended): copy `personal_settings.example.json` to `personal_settings.json` and fill the `vocabulary` block with the user's names, project terms, and frequent foreign words. Used by Soniox Live and the v4 path of the Soniox upload slot; the Groq APIs ignore it. Without the file, the tool simply runs unpersonalized.
- **Hotkeys:** the combinations live in the `HOTKEYS` dict in `config.py`. If a default collides with something the user already runs, change it there. For replacement combos, avoid special characters like `#` and non-ASCII letters (they can cause issues with the keyboard module); the default `ü` is established and known to work.
...
- **A hotkey does not register** (a `FAILED:` line at startup in the log) → another program already owns that combination — global hotkeys are exclusive in Windows → change the combo in the `HOTKEYS` dict in `config.py`.
