...
4. The self-test passes: the user presses `Ctrl+Alt+T`, and the bundled `test_audio.mp3` is transcribed and inserted at the cursor.
...
With only a Groq key, startup automatically skips the Soniox entries and starts on the first available API, announcing each skip. To start on Groq silently instead, prefer `"api": "groq-large"` in the `defaults` block of `personal_settings.json` over `DEFAULT_API = "groq-large"` in `config.py`: only the settings file counts as *configured*, so only it outranks the engine the tool remembers from the user's last `Ctrl+Alt+L` switch (the constant loses to a remembered engine). Ask the user, don't assume.
...
- **Recognition vocabulary** (recommended): copy `personal_settings.example.json` to `personal_settings.json` and fill the `vocabulary` block with the user's names, project terms, and frequent foreign words. The example's placeholder terms ("Project Name", "Acronym", …) become live vocabulary the moment the copy goes unedited — replace them all and drop what the user does not need, and do the same when the copy is made only for the hotkeys block below. Used by every Soniox engine — Soniox Live and the Soniox upload slot; the Groq APIs ignore it. Without the file, the tool simply runs unpersonalized.
- **Hotkeys:** rebind any combination in the `hotkeys` block of `personal_settings.json` (copy `personal_settings.example.json` first) — a partial override keyed by action name, with `config.py` keeping the defaults. If a default collides with something the user already runs, override it there. Keys are letters, digits, F-keys `f1`–`f24`, the navigation cluster (`insert`, `delete`, `home`, `end`, `pageup`, `pagedown`), the arrow keys (`left`, `up`, `right`, `down`), the numpad (`num0`–`num9`, `numdecimal`, `numdivide`, `nummultiply`, `numsubtract`, `numadd`), `pause` and `scrolllock` — any of them bare or with modifiers, with three technical exceptions, each rejected with a warning that names the mechanism: ctrl combined with `pause` or `scrolllock` (with Ctrl held, Windows delivers a different key code, so the combo could never fire), plain `ctrl+v` (the paste the tool itself sends to insert a transcript), and `ctrl+alt` with `q`, `e`, `m`, `2`, `3`, `7`, `8`, `9` or `0` (on German keyboards AltGr is Ctrl+Alt, so those are what typing `@ € µ ² ³ { [ ] }` sends). Anything else (special characters like `#`, non-ASCII letters) is not accepted — such keys can get typed into some apps and, having no fixed key code, would have to be resolved against the active layout at startup. A bad or colliding entry warns in the log and keeps the default — never a failed start. The startup engine is overridable the same way, in a `defaults` block (`"api"`). Note for what you tell the user: the tool also remembers the engine they last switched to with `Ctrl+Alt+L` (in its own `runtime_state.json`, written by the tool, safe to delete) and starts on it — but a `defaults.api` set here outranks that memory, so setting one pins the startup engine for good.
...
- **Push-to-talk** (opt-in, off by default): an alternative dictation gesture — tap Left-Ctrl, release, then right away press-and-hold Left-Ctrl; recording runs while held, releasing inserts. (The second press must follow within `tap_window_s`, 0.3 s by default — a slow repeat is not the gesture and does nothing.) Switch it on in the settings app, **Hotkeys** tab, where the gesture is explained and the choice sits ahead of the presets; the `push_to_talk` block of `personal_settings.json` stays the place for the trigger key, insert path (`clipboard` by default, `type` as the paste-blocked fallback), and timing thresholds — and `enabled: true` can still be set there by hand. Ask whether the user wants it — it reads every trigger press while enabled, so it is a deliberate choice. A mandatory AltGr filter keeps German QWERTZ characters (`@ \ { } [ ] | € ~`) from false-triggering. If the user works in a JetBrains IDE, mention that double-Ctrl is the IDE's "Run Anything" shortcut and they should enable "Disable double modifier key shortcuts" in the IDE's Advanced Settings to avoid the clash. Same elevated-window limit as the hotkeys (see Known failure modes).
...
2. Self-test: have the user focus a text field (plain Notepad works well), then press `Ctrl+Alt+T`. The tool transcribes the bundled `test_audio.mp3` through the active API and inserts the text at the cursor.
