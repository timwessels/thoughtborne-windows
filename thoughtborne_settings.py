...
"Save & restart") and the everyday settings dialog (rail: "Save & restart" /
...
# would append a block per tick, and these appends bypass the tool's log rotation
# (only the tool's own writes rotate it). Twenty blocks are plenty for any diagnosis.
_ERROR_LOG_CAP = 20

# Pixel gear for the masthead (#228): 15x15, six teeth, open
# hub; the gear cites the settings hotkey's own mnemonic ("G as in gear", #164).
# Generated by supersampling a clean vector gear (ring + 6 trapezoid teeth +
# round hub) down onto the grid, so it is exactly symmetric; "#" is a fully lit
# pixel, "+" a half-lit one -- the anti-aliased edge that keeps the shape
# readable at this size. Rendered by SettingsApp._pixel_canvas. Art, not
# prose -- edit like console_ui's logo marks, never "reformat".
_GEAR_PIXELS = (
    "......###......",
    ".....+###+.....",
    "..+..+###+..+..",
...
class SettingsApp:
    ...
    def __init__(self, root: tk.Tk, first_run: bool):
        ...
        # Crash reporting first (#240): the app runs with all three streams on DEVNULL
        # (#222/#227), so Tk's default handler -- a stderr print -- discards every
        # exception a widget callback raises. Hanging our own handler on the root as an
        # instance attribute is tkinter's documented override path, and doing it here
        # rather than in main() means every entry point that builds the real app (the
        # tool's spawn, the render harness, the display-gated ladder checks) is wired
        # the same way. The counter must exist before the wiring, since the handler
        # reads it. Anything raised outside a callback -- construction, mainloop
        # itself -- is main()'s try/except instead; the two lanes meet without a gap.
        ...
        # force (the fixed-mode radio list), "remember" means start on the engine last
        ...
        # The fixed-mode radio-list selection (#201). Seeded to the pin, else the
        ...
        # SELECTED in by the user is persisted, so merely displaying a remembered
        # engine never promotes it into a pin.
        ...
        # mode will start on before saving). Read-only -- nothing in the window moves
        # it, and the app writes no engine memory at all (D-028).
        ...
        # "currently remembered" vs "no switch recorded yet" wording.
        ...
        # var: since #328 it is the state a save compares the fields against, so a
        # field the user CLEARED can be told from one that was empty all along
        # (settings_io.resolve_env_save_updates, D-026). Never
    ...
    def _report_callback_exception(self, exc, val, tb):
        """Where Tk routes every exception a widget callback raises (#240).

        `CallWrapper.__call__` catches everything but SystemExit and hands it to
        `Misc._report_exception` -> `root.report_callback_exception`, which __init__
        pointed here. Covered by that route: command callbacks, bind handlers,
        `after`/`after_idle`, variable traces, the WM_DELETE_WINDOW protocol. NOT
        covered: the key-check worker thread (it contains itself by contract, #205)
        and anything raised outside a callback (main()'s wrap).

        Writing the block is the whole job. The window stays usable because Tk already
        consumed the exception at the callback boundary and keeps dispatching -- that
        is the unchanged default, not something this handler arranges. Delegating to
        the stock handler afterwards is additive: production sends its stderr to
        DEVNULL, but a console or Xvfb start keeps the traceback it prints today.
        Never raises (it cannot afford to, being the crash path) and touches no
        widget, so it can never trigger a callback of its own.
        """
        try:
            self._error_log_count += 1
            stamp = time.strftime("%Y-%m-%d %H:%M:%S")
            if self._error_log_count <= _ERROR_LOG_CAP:
                settings_visibility.append_log_line(
                    config.LOG_FILE, settings_visibility.format_error_block(
                        stamp, "callback", exc, val, tb))
            elif self._error_log_count == _ERROR_LOG_CAP + 1:
                settings_visibility.append_log_line(
                    config.LOG_FILE, settings_visibility.format_settings_line(
                        stamp, "error",
                        f"further callback errors suppressed after {_ERROR_LOG_CAP}"))
        except Exception:
            pass
        try:
            tk.Tk.report_callback_exception(self.root, exc, val, tb)
        except Exception:
            # The stock handler prints to sys.stderr, which is None in a bare pythonw
            # start -- its own docstring says applications should override it for
            # exactly that case. Guarded separately so the log write above stands
            # whatever the delegation does.
            pass
    ...
    def _section(self, parent, heading_key, level="H1", pady=(0, 0)):
        ...
        Terminal-style (#228): the heading sits in a row with a
    ...
    def _build_header(self):
        ...
        # The two radio labels name themselves and never translate. English
        # leads because it is the default language (D-015).
        ...
        ttk.Radiobutton(lang_frame, text=strings.t("lang.de", "de"), value="de",
                        variable=self.lang_var, command=self._on_lang).pack(side="left")
        # Terminal masthead (#228), one line: pixel gear +
    ...
    def _pixel_canvas(self, parent, bitrows, px, fill=TEXT_COLOR, dim=MUTED):
        """Render pixel-art rows as a canvas of square `px` cells. Rows hold
        pixel levels: booleans (half-block art) or 0/0.5/1 floats (the gear,
        whose half-lit pixels render in `dim` -- poor man's anti-aliasing).
        Square cells keep the terminal aspect for half-block art (a char cell
        is px wide, 2*px tall); sp()-derived px scales with DPI. A few hundred
        one-off rectangles -- drawn once, never touched again."""
    ...
    def _on_field_edit(self, provider):
        ...
        # disabled). That verdict is ALL an edit touches (D-028): it describes this
        # field's exact text, while nothing else in the window reads a key field
        # before the save.
    ...
    def _build_behavior_tab(self):
        ...
        # (#201's widget, replacing the #198 combobox): all four engines stay visible
        # so the user learns they exist, and all four are always selectable -- the
        # picker is key-agnostic (D-028). #155 keeps the whole control in a card.
        card = self._card(f)
        card.pack(fill="x")
        ...
        # One radio per engine. The label (engine name + descriptor) is language-
        # dependent and composed from two sources, so _render_engine_control builds it
        # by hand rather than _reg-istering a single key here. engine_var holds the
        # selected engine id; a programmatic .set() fires no command, so RENDERING the
        # selection is never a user pick, which is the basis of the D-002
        # byte-identity of an untouched save.
    ...
    def _render_engine_control(self):
        ...
        # language, then set its state. Enabled iff fixed-mode -- the engine list is
        # the "always start with" radio's own argument, so mode pair and list are one
        # composite control and the list is inactive as a whole in remember-mode --
        # and never by key state: the picker is key-agnostic (D-028). A pin on an
        # engine without a key is the user's call; the next start resolves it through
        # the carousel (#40/#200), which is why the window needs no guard of its own.
        # Setting engine_var shows the current selection; a programmatic set fires no
        # command, so this never counts as a user pick (D-002): engine_index stays
        # untouched, so an untouched save still writes byte-identically.
        ...
        for a in config.AVAILABLE_APIS:
            ...
            rb.state(["!disabled"] if mode_fixed else ["disabled"])
        self.engine_var.set(config.AVAILABLE_APIS[self.engine_index])

        # The remember label names the engine remember-mode will start on: the real
        # Ctrl+Alt+L memory when one exists, else the built-in default under the "no
        # switch recorded yet" wording. Read-only -- the settings app never writes the
        # memory (D-028 retired its one write lane, the #178 wizard preselect).
    ...
    def _on_engine(self):
        ...
        # radio -- in remember-mode the radios are disabled and ignore the click.
        # Track selection by index into AVAILABLE_APIS.
        self.engine_index = config.AVAILABLE_APIS.index(self.engine_var.get())

    def _on_mode(self):
        # Re-render so the engine radios enable/disable with the mode. A mode switch
        # never moves the selection itself (D-028 retired #207's keyed-engine
        # auto-move): what "always start with" pins is the engine the list shows,
        # keyed or not.
    ...
    def _has_any_key(self):
        ...
        stored keys, so clearing one removes it). The predicate behind the pre-save
        no-key warning in _save -- a check of the save GESTURE itself (D-028), which
        is why clearing the last key now meets the confirmation that guards saving
        without one. It no longer reaches the rail: since #271 every save restarts,
        keyed or not (D-014), so there is no label left for a key to decide."""
        return bool(self.groq_var.get().strip() or self.soniox_var.get().strip())

    def _live_env(self):
        """The two managed key fields as an {ENV_VAR: value} dict (config.API_KEY_ENV
        names those vars): the live half of the save's difference rule in
        _env_save_updates below, against the load-time _stored_env snapshot."""
    ...
    def update_rail(self, event=None):
        ...
        if idx < last:
            ...
            # The last tab's button IS the save button, and every save restarts (#271,
            # D-014) -- keyed or keyless. The tool is always running when the wizard is
            # open (#223/D-014: the keyless #200 shop window spawned this very wizard),
            # so no liveness probe either.
            key = "btn.save_restart"
    ...
    def _render_rail(self):
        ...
        if self.first_run:
            ...
            # Every save restarts (#271, D-014), so the label is fixed -- and the tool
            # is always running when this window is open (#223/D-014), so no liveness
            # probe and nothing left to explain in a footer: with one truth about when
            # changes apply the footer is gone.
            self.save_btn.config(text=strings.t("btn.save_restart", self.lang))
    ...
    def _save(self):
        ...
        # Engine field (#193/#198, D-008): derive the on-save signal from the two-mode
        # control in one pure, off-Windows-tested place. It is what defaults.api gets
        # -- None (leave as found) / REMOVE_API_PIN (drop the pin) / an id (write
        # verbatim, built-in default included, keyed or not -- D-028). The save writes
        # no engine memory: runtime_state.json records what the user switched to while
        # dictating, and this window never moves that.
        engine_now = config.AVAILABLE_APIS[self.engine_index]
        engine_loaded = config.AVAILABLE_APIS[self._engine_index_loaded]
        default_api_signal = settings_io.resolve_engine_save_signal(
        ...
        # Post-save action: every save restarts (#271, D-014). The settings are on
        # disk; the #202 handshake performs the start that picks them up (D-002 --
        # pickup is start-based, so a save that does not restart just defers itself
        # silently). Keyless included: the relaunched tool comes back as the #200 shop
        # window and opens this window again, which is that rule doing its job, not a
        # loop. If the tool has meanwhile died unclean (the only not-running case
...
def _bind_startup_timing(root, t_tk, t_size, t_construct, first_run):
    ...
    def _on_first_map(_evt):
        ...
        try:
            ...
            settings_visibility.append_log_line(
                config.LOG_FILE, settings_visibility.format_settings_line(
                    stamp, "startup", f"{' '.join(parts)} mode={mode}"))
        except Exception:
            pass
    ...
    def _on_first_expose(_evt):
        ...
        try:
            ...
            line = settings_visibility.format_visible_line(
                ...
                mode)
            settings_visibility.append_log_line(config.LOG_FILE, line)
        except Exception:
            pass
...
def main():
    try:
        ...
        parser.add_argument("--first-run", action="store_true",
                            help="force first-run wizard mode; the app also "
                                 "auto-promotes to it when no API key is stored yet "
                                 "(#163)")
        ...
        # notice (unlike the tool's D-004 refuse). Checked before tk.Tk() so no window
        # is built only to be discarded. settings_instance is fail-open: any guard
        # fault just starts normally, never costing a launch.
        ...
        if already:
            ...
            # no-op. Same sink as the startup line -- guarded in append_log_line since
            # #240 -- written before tk.Tk() in this second process.
            outcome = settings_instance.focus_existing_settings_window()
            settings_visibility.append_log_line(
                config.LOG_FILE, settings_visibility.format_settings_line(
                    time.strftime("%Y-%m-%d %H:%M:%S"), "focus-existing", outcome))
            sys.exit(0)

        # Import-time configuration warnings, into the log this window's user can be
        # pointed at (#240). config collects them instead of logging them (#206/#238),
        # and its replay goes through a logger -- which in this process has no handler
        # and would end on stderr, i.e. DEVNULL. So replay them here, through the same
        # sink: the settings window IS the repair surface for a broken .env or
        # personal_settings.json, and its own log trail should name what needs
        # repairing. Written after the single-instance guard (a focus-and-exit second
        # process should not repeat them) and before tk.Tk(), so they stand above any
        # error: block a construction fault writes next.
        for _msg in config.IMPORT_WARNINGS:
            settings_visibility.append_log_line(
                config.LOG_FILE, settings_visibility.format_settings_line(
                    time.strftime("%Y-%m-%d %H:%M:%S"), "import-warning", _msg))
        ...
        # when no readable key is stored yet -- so the installer hand-off, which passes
        # no flag, lands on the guided wizard on a fresh keyless machine, while a re-run
        # over a keyed install still opens the plain settings dialog. read_env never
        # raises.
        env = settings_io.read_env(config.SCRIPT_DIR / ".env")
        ...
        # Everything outside a widget callback: constructing the window, and whatever
        # mainloop() itself raises (#240). Without this, a failure here is a silent
        # no-op -- all three streams are DEVNULL (#222/#227) and the tool never reads
        # the exit code, so the log would say "Opened the settings app" and nothing
        # more, while Ctrl+Alt+G visibly did nothing. Log the reason, then re-raise:
        # production loses nothing by it, and a console start keeps its stderr
        # traceback and its non-zero exit. SystemExit (the focus-existing sys.exit(0))
        # and KeyboardInterrupt are BaseExceptions and pass through untouched. A
        # failure at import time is above this frame and out of reach by nature -- the
        # tool's own log line is the only trace of that one.
        settings_visibility.append_log_line(
            config.LOG_FILE, settings_visibility.format_error_block(
                time.strftime("%Y-%m-%d %H:%M:%S"), "main", *sys.exc_info()))
        raise
